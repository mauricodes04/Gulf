...
def create_chart(characteristicName: str, input_path: str) -> str:
    ...
    if not os.path.exists(input_path):
        print(f"\rError: File not found at {input_path}" + " " * 20)
        return
...
def fetch_API(inVar, inVar2):
    ...
    if response.status_code != 200:
        print(f"\rError Code: {response.status_code}" + " " * 20)
        return ""
...
# 1) load values
    ...
    print("\rCreating chroma value storage..." + " " * 20, end='', flush=True)
...
def __init__():
    ...
    print(f"\r<dev> Found {len(results_list)} related characteristics" + " " * 20)
